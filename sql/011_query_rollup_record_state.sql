...
-- Forward-only hot-path optimization for the checksum-frozen 0008 migration.
--
-- Keep one timestamp-ordered array of native PoWA records per chunk and derive
-- both boundary states from it.  This avoids constructing and sorting two
-- equivalent JSONB aggregate states for every chunk while preserving the
-- counter, reset, gap, predecessor and multi-user coverage semantics.
...
WITH bounds AS (
    ...
    CROSS JOIN LATERAL (
        ...
        SELECT
            ...
            (array_agg(sample_state ORDER BY sample_at))[1] AS first_state,
            (array_agg(sample_state ORDER BY sample_at))[
                cardinality(array_agg(sample_state ORDER BY sample_at))
            ] AS last_state,
    ...
    SELECT
        ...
        previous_chunk_sample_at IS NOT NULL AND COALESCE(
            (first_state).calls
                < (previous_chunk_state).calls
            OR (first_state).total_exec_time
                < (previous_chunk_state).total_exec_time
            OR (first_state).shared_blks_hit
                < (previous_chunk_state).shared_blks_hit
            OR (first_state).shared_blks_read
                < (previous_chunk_state).shared_blks_read
            OR (first_state).temp_blks_written
                < (previous_chunk_state).temp_blks_written
            OR (first_state).wal_bytes
                < (previous_chunk_state).wal_bytes
            OR (first_state).rows
                < (previous_chunk_state).rows,
            false
    ...
    SELECT
        ...
        sum(boundary.current_calls + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).calls
                        >= (boundary.previous_chunk_state).calls
                    THEN (boundary.first_state).calls
                        - (boundary.previous_chunk_state).calls
                ELSE (boundary.first_state).calls
            END
            ELSE 0
        END)::bigint AS calls,
        sum(boundary.current_rows + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).rows
                        >= (boundary.previous_chunk_state).rows
                    THEN (boundary.first_state).rows
                        - (boundary.previous_chunk_state).rows
                ELSE (boundary.first_state).rows
            END
            ELSE 0
        END)::bigint AS rows,
        sum(boundary.current_total_exec_time_ms + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).total_exec_time
                        >= (boundary.previous_chunk_state).total_exec_time
                    THEN (boundary.first_state).total_exec_time
                        - (boundary.previous_chunk_state).total_exec_time
                ELSE (boundary.first_state).total_exec_time
            END
            ELSE 0
        ...
        sum(boundary.current_shared_blocks_hit + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).shared_blks_hit
                        >= (boundary.previous_chunk_state).shared_blks_hit
                    THEN (boundary.first_state).shared_blks_hit
                        - (boundary.previous_chunk_state).shared_blks_hit
                ELSE (boundary.first_state).shared_blks_hit
            END
            ELSE 0
        ...
        sum(boundary.current_shared_blocks_read + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).shared_blks_read
                        >= (boundary.previous_chunk_state).shared_blks_read
                    THEN (boundary.first_state).shared_blks_read
                        - (boundary.previous_chunk_state).shared_blks_read
                ELSE (boundary.first_state).shared_blks_read
            END
            ELSE 0
        ...
        sum(boundary.current_temp_blocks_written + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).temp_blks_written
                        >= (boundary.previous_chunk_state).temp_blks_written
                    THEN (boundary.first_state).temp_blks_written
                        - (boundary.previous_chunk_state).temp_blks_written
                ELSE (boundary.first_state).temp_blks_written
            END
            ELSE 0
        ...
        sum(boundary.current_wal_bytes + CASE
            ...
            THEN CASE
                WHEN (boundary.first_state).wal_bytes
                        >= (boundary.previous_chunk_state).wal_bytes
                    THEN (boundary.first_state).wal_bytes
                        - (boundary.previous_chunk_state).wal_bytes
                ELSE (boundary.first_state).wal_bytes
            END
            ELSE 0
